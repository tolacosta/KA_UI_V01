		    	            	...
		    	            	if(data == "Bad credentials"){
		    	            		$("#message").replaceWith('<div id="message" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
		    	            		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
										  				   ...
										  				   '<strong class="alert-link">អ៊ីមែល​ ឬលេខសម្ងាត់មិនត្រឹមត្រូវទេ! សូមព្យាយាមម្តងទៀត!</strong>'+ 
														   '</div>');
		    	            	}else if(data == 'false'){
		    	            		$("#message").replaceWith(
		    	            				   ...
		    	            				   '<div id="message" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
	            		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
							  				   '<strong class="alert-link">អ៊ីមែលនេះធ្លាប់បានចុះឈ្មោះដើម្បីបង្កើតគណនីរួចហើយ​ ប៉ុន្តែគណនីនេះមិនអាចដំណើរការបានទេ! សូមពិនិត្យមើលអ៊ីមែលរបស់អ្នកឡើងវិញ ។ ប្រសិនជាអ្នកមិនបានទទួលសារនៅក្នុងប្រអប់សាររបស់អ្នកទេ សូមពិនិត្យមើលប្រអប់​ Spam ឬ </strong>'+ 
											   '<br/><a href="#" id="btFrmSendMailToConf">ស្នើរសុំសារថ្មីមួយទៀត!</a>'+
							  				   '</div>'
							  				   );
		    	            	}else{
									$("#message").replaceWith('<div id="message" class="alert alert-success alert-bold-border square fade in alert-dismissable"> '+ 
 	            		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
								  				  ...
								  				  '<strong class="alert-link">ចូលប្រើប្រាស់បានជោគជ័យ!</strong>'+ 
												   '</div>');
		    	            		setTimeout(function(){
		    	            			location.href = data;
		    	            		}, 500 );
	         		  ...
	         		  if( $("#password").val().length < 6 || $("#repassword").val().length < 6){
	         			 $("#message-re").replaceWith('<div id="message-re" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
	  		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
					  				  ...
					  				  '<strong class="alert-link">សូមបំពេញលេខសម្ងាត់ចាប់ពី ៦ ខ្ទង់ឡើងទៅ!</strong>'+ 
									   '</div>');
		         			  return; 
	         		  }
	         			 ...
	         			 $("#message-re").replaceWith('<div id="message-re" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
  		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
				  				  /*  '<strong class="alert-link">Passwords do not match!</strong>'+  */
				  				   '<strong class="alert-link">បញ្ជាក់លេខសម្ងាត់មិនត្រឹមត្រូវទេ!</strong>'+ 
								   '</div>');
	         			  return;
	         		  }
		    	            ...
		    	            success: function(data) { console.log(data);
		    	            	...
		    	            	if(data.STATUS == "NOTCONFIRMED"){
		    	            		$("#message-re").replaceWith('<div id="message-re" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
				   		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
												   ...
												   '<strong class="alert-link">អ៊ីមែលនេះធ្លាប់បានចុះឈ្មោះដើម្បីបង្កើតគណនីរួចហើយ​ ប៉ុន្តែគណនីនេះមិនអាចដំណើរការបានទេ! សូមពិនិត្យមើលអ៊ីមែលរបស់អ្នកឡើងវិញ ! '+
												   ' ប្រសិនជាអ្នកមិនបានទទួលសារនៅក្នុងប្រអប់សាររបស់អ្នកទេ សូមពិនិត្យមើលប្រអប់ Spam ឬ' +
												   ' <a href="#resend" id="btFrmSendMailToConf" class="btn btn-primary btn-xs">ស្នើរសុំសារថ្មីមួយទៀត </a>' + 
				 							   '</div>');
		    	            		KA.destroyProgressBar();
		    	            	}
		    	            	// Email is already register with facebook
		    	            		...
		    	            		$("#message-re").replaceWith('<div id="message-re" class="alert alert-danger alert-bold-border square fade in alert-dismissable"> '+ 
				   		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
				 				  				   ...
				 				  				   '<strong class="alert-link">អ៊ីមែលនេះធ្លាប់បានចុះឈ្មោះរួចហើយ! សូមប្រើប្រាស់អ៊ីមែលដ៏ទៃទៀត ឬ   <a href="#reset" id="btFrmreset">កំណត់លេខសម្ងាត់ឡើងវិញ!</a></strong>'+ 
				 							   '</div>');
		    	            		KA.destroyProgressBar();
		    	            	}
		    	            	// Email is already register
		    	            		...
		    	            		$("#message-re").replaceWith('<div id="message-re" class="alert alert-success alert-bold-border square fade in alert-dismissable"> '+ 
				 					               ...
				 					               '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
				 				  				   '<strong class="alert-link">គណនីរបស់អ្នកត្រូវបានបង្កើត។ សូមពិនិត្យមើលអ៊ីមែលរបស់អ្នកដើម្បីធ្វើអោយគណនីរបស់អ្នកអាច ដំណើរការ និងចូលប្រើបា្រស់ Khmer Academy បាន ។ <br/> '+
				 					               'ប្រសិនជាអ្នកមិនបានទទួលសារនៅក្នុងប្រអប់សាររបស់អ្នកទេ សូមពិនិត្យមើលប្រអប់Spam ឬ <a href="#resend" id="btFrmSendMailToConf" class="btn btn-primary btn-xs">ស្នើរសុំសារថ្មីមួយទៀត </a> ។'+
				 					               '</strong>'+ 
				 				  				   '</div>');
								
								
									$("#pop-re").replaceWith( '<strong style="font-size: 18px;" class="alert-link">គណនីរបស់អ្នកត្រូវបានបង្កើត។ សូមពិនិត្យមើលអ៊ីមែលរបស់អ្នកដើម្បីធ្វើអោយគណនីរបស់អ្នកអាច ដំណើរការ និងចូលប្រើបា្រស់ Khmer Academy បាន ។</strong>'+
															  '<p style="font-size: 14px;color:white">ប្រសិនជាអ្នកមិនបានទទួលសារនៅក្នុងប្រអប់សាររបស់អ្នកទេ សូមពិនិត្យមើលប្រអប់Spam ឬ <a href="#resend" id="btFrmSendMailToConf" class="btn btn-primary btn-xs">ស្នើរសុំសារថ្មីមួយទៀត </a></p>');
 						...
 						success: function(data){ 
 							$("#message-resend").replaceWith('<div id="message-re" class="alert alert-success alert-bold-border square fade in alert-dismissable"> '+ 
		   		                       '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>'+ 
		 				  				   ...
		 				  				   '<strong class="alert-link">ក្រុមរបស់យើងបានផ្ញើសារទៅកាន់អ៊ីមែលរបស់អ្នក ។  សូមពិនិត្យមើលអ៊ីមែលរបស់អ្នកដើម្បីធ្វើអោយគណនីរបស់អ្នកអាច ដំណើរការ និងចូលប្រើបា្រស់ Khmer Academy បាន ។ ​ប្រសិនជាអ្នកមិនបានទទួលសារនៅក្នុងប្រអប់សាររបស់អ្នកទេ សូមពិនិត្យមើលប្រអប់ Spam ឬ ស្នើរសុំសារថ្មីមួយទៀត ។ </p>'+ 
		 					            '</div>');
		    	            	KA.destroyProgressBarWithPopup();
		    	            },
		    	         	error: function(data){
